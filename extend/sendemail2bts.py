...
if __name__ == '__main__':
    ...
    rom_p = 'https://teleweb-sz.tclcom.com/0_Shenzhen/teleweb'
    ...
    if isdelivery == 'true':
        content = zip_project + '/' + zip_type + '/' + zip_version \
                    + ' 打包成功.\n\n' \
                    + 'BTS路径:' \
                    + rom_p + '/' + zip_project + '/' + zip_type + '/' + zip_version \
                    + "\n\n"

        subject = '%s-%s BTS打包' % (zip_project, zip_version)
    else:
        content = zip_project + '/' + zip_type + '/' + zip_version + ' BTS打包失败,请告知集成组同事处理,谢谢!'
        subject = '%s-%s BTS打包' % (zip_project, zip_version)
